...
func Init() error {
	...
	if os.Getenv("SQL_DRIVER") == "postgres" {
		db, err = initpostgres()
	} else {
		db, err = initmysql()
	}
...
func initpostgres() (*gorm.DB, error) {
	dsn := "host=10.13.128.30 user=postgresUser password=postgresPW dbname=postgresDB port=5455 sslmode=disable TimeZone=Asia/Shanghai"
